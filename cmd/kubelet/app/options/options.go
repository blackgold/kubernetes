...
func AddKubeletConfigFlags(mainfs *pflag.FlagSet, c *kubeletconfig.KubeletConfiguration) {
	...
	fs.DurationVar(&c.PodTerminatedStatePeriod.Duration, "pod-terminated-state-period", c.PodTerminatedStatePeriod.Duration, "Do not delete the pod for a grace period after the pod reaches a terminated state. This ensures kubelet doesnot delete infra container immediately after pod reaches terminated state. This gives time for network plugin to release devices from infra container.")
}
